...
const gameInviteService = {
  ...
  invitePlayer: async (senderId, receiverId) => {
    ...
    if (senderId.toString() === receiverId.toString()) {
      throw new ApiError(
        ...
        "You cannot invite yourself to play",
      );
    }

    const receiverLastActive = receiver.lastActive
      ? new Date(receiver.lastActive).getTime()
      : 0;
    const isReceiverOnline =
      Date.now() - receiverLastActive < ONLINE_THRESHOLD_MS;
    ...
    if (receiver.activity === "playing") {
      throw new ApiError(
        HTTP_STATUS.BAD_REQUEST,
        "Friend is currently playing another match",
      );
    }

    // 5. Create pending invite
    const invite = await gameInviteRepository.createInvite(
      senderId,
      receiverId,
    );
    return invite;
  ...
  respondToInvite: async (userId, inviteId, action) => {
    ...
    const isExpired =
      new Date(invite.createdAt) < new Date(Date.now() - 5 * 60 * 1000);
    if (isExpired) {
    ...
    if (sender.activity === "playing") {
      throw new ApiError(
        HTTP_STATUS.BAD_REQUEST,
        "Sender is already in another game",
      );
    }
    if (receiver.activity === "playing") {
      throw new ApiError(
    ...
    const game = await GameManager.initGame(
      room.roomId,
      invite.sender,
      invite.receiver,
    );
    ...
    if (senderSocketId && StateSynchronizer.io) {
      StateSynchronizer.io
        .to(senderSocketId)
        .emit("invite-accepted", { matchId: room.roomId });
    }
  ...
  leaveGame: async (userId) => {
    ...
    if (activeRoom) {
      ...
      if (wasActive) {
        ...
        const opponentId = activeRoom.players.find(
      ...
      const opponentStatus =
        PlayerManager.getPlayer(opponentId)?.status || "idle";
